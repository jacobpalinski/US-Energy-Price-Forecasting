...
def drop_columns(**context):
    ...
    latest_extracted_file_path = metadata.get('natural_gas_monthly_variables', {}).get('latest_extracted_file_path')

    # Retrieve extracted data from S3 folder
    natural_gas_monthly_variables_json = s3.get_data(s3_key=latest_extracted_file_path)
    ...
    try:
        schema.validate(natural_gas_monthly_variables_df)
    ...
    # Put data in S3
    s3.put_data(data=natural_gas_monthly_variables_df, s3_key=f'full_program/transformation/natural_gas_monthly_variables/natural_gas_monthly_variables_{ts_nodash}.json')
    s3_metadata.update_metadata(s3_key='full_program/metadata/metadata.json', dataset_key='natural_gas_monthly_variables', latest_transformed_file_path=f'full_program/transformation/natural_gas_monthly_variables/natural_gas_monthly_variables_{ts_nodash}.json', latest_transformed_timestamp=ts_nodash)
