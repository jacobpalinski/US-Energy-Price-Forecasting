...
class S3:
    ...
    s3_client (BaseClient): Connection for S3
    ...
    def get_data(self, s3_key: str) -> dict:
        ...
        '''
        try:
            logger.info(f'Retrieving data from the following S3 Bucket: {self.bucket} and S3 Key: {s3_key}')
            ...
            logger.exception(f'Error retrieving {s3_key}: {e}')
            return {}
    ...
    def put_data(self, data: list, s3_key: str) -> None:
        ...
        Args:
            ...
            s3_key (str): The S3 key for the object to be uploaded

        '''
        try:
            logger.info(f'Putting data to the following S3 Bucket: {self.bucket} and S3 Key: {s3_key}')
            ...
            logger.exception(f'Error putting data to {s3_key}: {e}')
            raise e
...
class S3Metadata(S3):
    ...
    secret_access_key (str): AWS Secret Access Key
    bucket (str): AWS S3 bucket for storage + retrieval
    s3_client (BaseClient): Boto3 client connection for S3

    Methods
    -------
    get_metadata(s3_key) -> dict:
        Retrieves metadata from S3, including multiple dates for each dataset 
    update_metadata(s3_key, dataset_key, **kwargs) -> None:
    ...
    def get_metadata(self, s3_key: str) -> dict:
        ...
        Returns:
            ...
            latest_extracted_file_path and latest_transformed_file_path keys amongst others.
    ...
    def update_metadata(self, s3_key, dataset_key, **kwargs) -> None:
        ...
        Args:
            s3_key (str): The S3 key where the metadata is stored
            dataset_key (str): The identifier of the dataset for which metadata is being updated
            **kwargs: Additional keyword arguments to handle latest start date, latest end date, latest extracted file path and latest transformed file path etc.
        '''
        logger.info(f'Updating metadata for dataset key: {dataset_key}')
