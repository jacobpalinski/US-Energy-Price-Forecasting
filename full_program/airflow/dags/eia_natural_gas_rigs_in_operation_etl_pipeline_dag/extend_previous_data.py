...
def extend_previous_data():
    ...
    latest_extracted_timestamp = metadata.get('natural_gas_rigs_in_operation', {}).get('latest_extracted_timestamp')
    latest_transformed_timestamp = metadata.get('natural_gas_rigs_in_operation', {}).get('latest_transformed_timestamp')
    previous_transformed_timestamp = metadata.get('natural_gas_rigs_in_operation', {}).get('previous_transformed_timestamp')
    ...
    natural_gas_rigs_in_operation_transformed_df = EtlTransforms.json_to_df(data=natural_gas_rigs_in_operation_transformed_json, date_as_index=True)
    ...
    if previous_transformed_timestamp is not None:
        if latest_extracted_timestamp > previous_transformed_timestamp:
            ...
            natural_gas_rigs_in_operation_previous_transformed_df = EtlTransforms.json_to_df(data=natural_gas_rigs_in_operation_previous_transformed_json, date_as_index=True)
            natural_gas_rigs_in_operation_df = pd.concat([natural_gas_rigs_in_operation_previous_transformed_df, natural_gas_rigs_in_operation_transformed_df])
        else:
            # If latest extracted timestamp is not later than previous transformed timestamp, retain latest transformed dataset as natural_gas_rigs_in_operation_df
            natural_gas_rigs_in_operation_df = natural_gas_rigs_in_operation_transformed_df
    else:
        # If latest extracted timestamp is not later than previous transformed timestamp, retain latest transformed dataset as natural_gas_rigs_in_operation_df
        natural_gas_rigs_in_operation_df = natural_gas_rigs_in_operation_transformed_df
    
    # Log row count of final dataset after concatenation
    logger.info(f"Final dataset after concatenation contains {len(natural_gas_rigs_in_operation_df)} rows")
    ...
    s3_metadata.update_metadata(s3_key='full_program/metadata/metadata.json', dataset_key='natural_gas_rigs_in_operation', previous_transformed_file_path=latest_transformed_file_path,
